...
<?php
if (isset($_GET['id'])) {
	$qry = $conn->query("SELECT * from orgnaization_bio where org_id= " . $_GET['id']);
...
<div class="container-field">
	<div class="col-lg-12">
		...
		<div class="row">
			...
			<div class="col-md-6">
				<!-- <p>Gender: <b><?php //echo $gender  -->
									?></b></p> -->
				<p>Account Status: <b><?php echo $org_status == 1 ? '<span class="badge badge-primary">Verified</span>' : '<span class="badge badge-secondary">Unverified</span>' ?></b></p>
			</div>
		</div>
	</div>
</div>
...
<script>
	$('.update').click(function() {
		start_load()
		$.ajax({
			url: 'ajax.php?action=update_org_acc',
			method: "POST",
			data: {
				id: <?php echo $org_id ?>,
			...
			success: function(resp) {
				if (resp == 1) {
					alert_toast("Organization account status successfully updated.")
					setTimeout(function() {
						location.reload()
					}, 1000)
				}
