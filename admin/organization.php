					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								...
								// code by Hamza start *******************
								$org = $conn->query("SELECT * from orgnaization_bio order by name asc");
								// code by Hamza end **********************
								// print(var_dump($org));
								// exit();
								...
								?>
									<tr>
										...
										<td class="">
											<p> <b><?php //echo $row['course'] 
													?>
													<?php if ($row['org_type'] == 1) : ?>
														<span class="badge badge-success">Academia</span>
													<?php else : ?>
										...
										<td class="text-center">
											<button class="btn btn-sm btn-outline-primary view_org" type="button" data-id="<?php echo $row['org_id'] ?>">View</button>
											<button class="btn btn-sm btn-outline-danger delete_org" type="button" data-id="<?php echo $row['org_id']; ?>">Delete</button>
										</td>
									</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	function delete_org($id) {
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					alert_toast("Organization successfully deleted", 'success')
					setTimeout(function() {
						location.reload()
					}, 1500)
	...
	$('.view_org').click(function() {
		uni_modal("Organization Bio", "view_org.php?id=" + $(this).attr('data-id'), 'mid-large')
